...
新值更新获取时间；相同值仅更新最近收到时间，不延长有效期。292 字节值优先按 URL-safe Base64 解码后的 Fernet 版本（0x80）和大端生成时间按账号配置的有效期计算到期时间（默认 60 分钟）；时间解析不验证签名。无法解析的格式按获取时间估算，未来时间最多按获取时间加配置有效期计算。重启恢复时重新计算到期时间，过期值不再用于自动请求。其他长度和缺失值保留原缓存。
...
按账号与实际上游模型分别调度，不区分思考强度；首次无值立即尝试；重新探测间距按账号配置，默认从票据生成时间起 40 分钟后开始续期。
配置的有效期只控制本地缓存，不代表上游验证结果。相同值不续期，获取时间仍表示本系统实际收到时间。
上游 HTTP 400/401/403/404 记录失败后继续探测，不暂停获取器；旧版因这些状态暂停的任务自动恢复。429/503 仍遵守冷却，账号、凭据及代理配置的本地不可用状态仍需处理。

Responses 的 OAuth 账号选择在同权重、可调度的候选之间优先使用持有对应上游模型有效 292 的账号。
指定账号、会话绑定、续接绑定、账号权重及额度/并发/冷却限制保持原优先级；优先组内部沿用原选择策略，
没有可用优先账号时回退到原候选。API Key 选择和没有模型的原生端点不受此规则影响。

普通 Responses 请求只要上游返回 312 字节 Turn State 就拦截，不受账号模式、获取器是否启用或是否已有有效缓存影响。
下游尚未提交时返回 HTTP 503，错误码 `turn_state_unavailable`，不交付该次上游结果；请求已发出，不自动重放可能产生副作用的工具调用。
`Retry-After` 按同账号/模型连续拦截次数依次为 30、60、180、300、600 秒，之后保持 600 秒。
未启用获取器、非自动模式或配置不可读取时使用 30 秒。获得有效 292 或获取器配置版本改变后重置；计数在内存中，服务重启也重置。后台获取器按自身出口调度和冷却规则重试。
...
`refreshIntervalMinutes` 默认 40 分钟，`stateTtlSeconds` 默认 3600 秒；有效期范围为 1–86400 秒，重新探测间距为正整数分钟，换算为秒后不能超过有效期。迁移 0024 将已有分钟配置乘以 60，保持实际有效期不变。
两者从票据生成时间（未知格式从首次获取时间）计算，重复票据不续期；修改有效期会重算已有缓存。有效期只控制网关停用时间，不延长上游认可时间。
...
探测不占用账号业务并发，也不受账号业务并发或请求间隔限制；账号状态校验及动态出口的并发/间隔限制仍然生效。Azure 同一出口服务内仍串行管理公网 IP。
...
静态出口保持单并发；动态 SOCKS5 出口按实例的 `maxConcurrent` 和 `intervalSeconds` 共享调度，不占用或受限于账号业务并发。静态出口未获得有效新值时从尝试结束起等待 10 秒。
...
整个探测过程不占用账号业务并发；动态 IP 分配后重新核对配置、账号和额度。每次重试获得新租约。
