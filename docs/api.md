...
新值更新获取时间；相同值仅更新最近收到时间，不延长有效期。292 字节值优先按 URL-safe Base64 解码后的 Fernet 版本（0x80）和大端生成时间计算生成后 60 分钟到期；时间解析不验证签名。无法解析的格式按获取时间估算，未来时间最多按获取后 60 分钟计算。重启恢复时重新计算到期时间，过期值不再用于自动请求。其他长度和缺失值保留原缓存。
...
按账号与实际上游模型分别调度，不区分思考强度；首次无值立即尝试，到期前 20 分钟开始续期。
60 分钟为约定的有效期，不代表上游验证结果。相同值不续期，获取时间仍表示本系统实际收到时间。
...
全局并发为 1，复用账号并发限制，账号繁忙则延后。未返回可用的新 292 字节值时，从本次尝试结束起固定等待
10 秒重试，不随失败次数增加间隔，不附加随机延迟。任务每秒检查一次，排队及新 IP 就绪可能增加实际等待时间。
连接、动态出口分配等错误仍按 1、2、5、10、15 分钟加少量随机延迟重试，
