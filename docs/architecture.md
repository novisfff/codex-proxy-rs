...
Codex Turn State 模式和手动值存于各 OpenAI 账号的 Provider 凭据中，随凭据 revision 更新。
选号后读取对应账号的配置，在账号状态隔离完成后应用；全局运行设置不再决定请求头模式。
默认仍遵循客户端状态的账号归属；显式手动或自动模式允许覆盖该状态。自动模式的最新 292 字节值由
OpenAI Provider 按实际账号 ID 与发送的上游模型名在进程内隔离，同一组合跨会话复用，不区分思考强度。
响应头与 WS 元数据仅更新本次请求的组合；自动模式未命中或已到期时清除原始透传状态。
`TurnStateStore` 持久化配置、最近值与尝试结果，Provider 启动恢复缓存，后台周期写入正常流量的新观测。
相同值只更新最近观测时间；按首次获取后 60 分钟估算过期，40 分钟开始续期。
OpenAI 获取器贡献独立的低并发 worker，使用专用连接池与显式代理，复用凭据和真实账号并发租约。
动态出口通过独立宿主服务管理 Azure 公网 IP，网关仅持有控制面客户端和一次性 CONNECT 租约。
等待地址就绪时不持有账号并发租约；拿到地址后重新校验账号和配置，使用不复用、不重试的 HTTP 客户端直连官方 OpenAI。
出口服务独占专用 NIC IP configuration，不修改业务主 IPv4；SQLite 负责全实例 24 小时去重、资源对账和重启恢复。
服务使用 Managed Identity，控制面和隧道分别鉴权；清理或出站验证失败时关闭分配，不允许回退。
配置 revision 隔离旧任务，更新配置取消在途请求；获取任务不调用通用客户计费链路。
探测复用缓冲执行存储记录 `model_requests`，以 `turn_state_fetcher` 标记内部请求，不关联客户 Key。
内部消费者完整读到成功终态后计入使用统计，是否获得 292 不影响计量；用量与费用沿用正常请求的响应解码器。
失败、流中断和取消保留执行审计，不计入成功用量；探测请求不保存正文或代理凭据。
动态获取按出口实例限制并发和启动间隔，允许同账号同模型并发搜索；结果提交串行校验配置与缓存，成功后取消其余搜索，防止迟到失败覆盖新值。
NovaProxy 每个租约独立创建，并通过 SOCKS5 用户名/密码认证建立 CONNECT；Azure 资源申请与清理在同一宿主出口服务内保持串行。
详细接口和退避语义见 [292 获取器](api.md#292-获取器)。
