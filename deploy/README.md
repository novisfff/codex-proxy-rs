...
自动缓存由数据库持久化，重启后恢复已保存的值，过期值不再携带。该凭据 schema 新增了配置字段，旧版本不识别；
...
在左侧“292 获取器”中为 OpenAI 账号选择模型及独立出口（静态代理、直连或动态 IP），然后开启自动获取。
账号配置中的“探测时段”默认全天，也可指定每天的北京时间窗口，支持跨午夜，例如 09:00–次日 01:00。
开始时间包含、结束时间不包含；时段外不发起探测，到结束时间取消进行中的探测及动态 IP 申请。
已有有效 Turn State 仍可用于正常请求；排队任务等到下一个开启时段再执行。
账号正常调用是否使用获取结果，仍由“账号管理 → 编辑 → X-Codex-Turn-State → 自动”决定。
获取器会消耗上游额度，默认关闭；使用极短请求，剩余约 20 分钟时开始刷新。
页面可以复制当前值，查看预计到期时间和失败原因。失败暂停时，修复账号、网关或代理后重新保存配置。
迁移 `0018` 增加配置、缓存和最近尝试表，`0019` 增加账号动态出口选择；完整数据库备份包含这些数据。
动态出口需要额外安装 [Azure 专用出口服务](dynamic-egress/README.md)，支持 IPv4 / IPv6。
返回 292 时保留 IP，下一次同一出口实例及 IP 协议优先复用；未返回 292 则释放并在下一次申请新 IP，
新申请避免与上一次地址相同。其 SQLite IP 状态需要独立备份，不包含在网关 PostgreSQL 备份中。
